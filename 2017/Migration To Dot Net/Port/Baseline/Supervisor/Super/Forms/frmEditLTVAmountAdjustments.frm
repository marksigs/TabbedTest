...
Begin VB.Form frmEditLTVAmountAdjustments 
   Caption         =   "Add/Edit LTV Amount Adjustments"
   ClientHeight    =   3195
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   4680
   Icon            =   "frmEditLTVAmountAdjustments.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   3195
   ScaleWidth      =   4680
   ...
   Begin MSGOCX.MSGComboBox cboProductCategory 
      ...
      Text            =   ""
   End
   Begin MSGOCX.MSGEditBox txtMaximumLTV 
      Height          =   285
      Left            =   1800
      TabIndex        =   1
      Top             =   480
   ...
   End
End
Attribute VB_Name = "frmEditLTVAmountAdjustments"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Form          : frmEditLTVAmountAdjustments
' Description   :
'
' Change history
...
Private m_clsProcFeeAdjByLTVTable As ProcFeeAdjByLTVTable
...
Dim clsTableAccess As TableAccess
    ...
    Set clsTableAccess = m_clsProcFeeAdjByLTVTable
    clsTableAccess.SetKeyMatchValues colMatchValues
    g_clsHandleUpdates.SaveChangeRequest m_clsProcFeeAdjByLTVTable
...
Private Sub Form_Load()
    ...
    Set m_clsProcFeeAdjByLTVTable = New ProcFeeAdjByLTVTable
...
Const c_strTHIS_FUNCTION As String = "PopulateScreenFields"
    ...
    g_clsFormProcessing.HandleComboExtra cboProductCategory, m_clsProcFeeAdjByLTVTable.GetProductCategory, SET_CONTROL_VALUE
    
    txtMaximumLTV.Text = m_clsProcFeeAdjByLTVTable.GetMaxLTV
    txtFeeRate.Text = m_clsProcFeeAdjByLTVTable.GetFeeRate
...
Dim clsTableAccess As TableAccess
    ...
    If Not m_bIsEdit Then
        ...
        m_colKeys.Add txtMaximumLTV.Text
        
        clsTableAccess.SetKeyMatchValues m_colKeys
        
        'Create a new record.
        g_clsFormProcessing.CreateNewRecord m_clsProcFeeAdjByLTVTable
    End If
    
    If cboProductCategory.ListIndex > -1 Then
        m_clsProcFeeAdjByLTVTable.SetProductCategory cboProductCategory.GetExtra(cboProductCategory.ListIndex)
    End If
    m_clsProcFeeAdjByLTVTable.SetMaxLTV txtMaximumLTV.Text
    m_clsProcFeeAdjByLTVTable.SetFeeRate txtFeeRate.Text
...
Dim colValues As New Collection
    ...
    Set objTableAccess = m_clsProcFeeAdjByLTVTable
...
Private Sub EnableOK()
    cmdOK.Enabled = (cboProductCategory.ListIndex > -1 And Val(Format$(txtMaximumLTV.Text)) > 0 And Len(txtFeeRate.Text) > 0)
...
Private Sub txtMaximumLTV_Change()
    Call EnableOK
End Sub


Private Sub txtMaximumLTV_Validate(Cancel As Boolean)
    Cancel = Not txtMaximumLTV.ValidateData()
End Sub
