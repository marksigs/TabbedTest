...
Begin VB.Form frmFindAssociation 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Find Association"
   ClientHeight    =   6135
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   8790
   Icon            =   "frmFindAssociation.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6135
   ...
   End
End
Attribute VB_Name = "frmFindAssociation"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Form          : frmFindAssociation
' Description   : Finds all introducers matching the search criteria
'
' Change history
' Prog      Date        Description
' PB        03/11/06    EP2_13 - Copied from frmFindFirmBroker to frmFindAssociation
'                       and modified to use for Association.
...
Private m_objAssociationTable As AssociationTable

Public Function GetTableClass() As IntroducerTable
    Set GetTableClass = m_objAssociationTable
End Function
Public Sub SetTableClass(TableClass As AssociationTable)
    Set m_objAssociationTable = TableClass
End Sub
...
Private Sub Form_Initialize()
    ...
    Set m_objAssociationTable = New AssociationTable
    '
End Sub
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Private Sub FindAssociation()
    
    Dim objAssociationSearch As AssociationSearch
    
    On Error GoTo Failed
    
    'Create a class used to search.
    Set objAssociationSearch = New AssociationSearch
    
    'Set the Name criteria.
    If Me.txtFirmName.Enabled = True Then
        objAssociationSearch.SetName Me.txtFirmName.Text
    End If
    
    'Set address
    If Me.txtFirmAddress.Enabled Then
        objAssociationSearch.SetAddress Me.txtFirmAddress.Text
    End If
    
    'Perform a search and populate the table object.
    objAssociationSearch.Find m_objAssociationTable
...
Private Sub PopulateLV()
    ...
    If TableAccess(m_objAssociationTable).RecordCount > 0 Then
        For nThisInt = 1 To TableAccess(m_objAssociationTable).RecordCount
            Set lvItem = lvResults.ListItems.Add
            
            'Unique key
            lvItem.Key = "ASSOC" & m_objAssociationTable.GetAssociationId ' GetIntroducerGUID
            
            'Because the key can only be a string value, the id must be stored in the tag as a byte()
            lvItem.Tag = m_objAssociationTable.GetAssociationId
            
            lvItem.Text = m_objAssociationTable.GetName
            
' TW 16/01/2007 EP2_859
'            lvItem.ListSubItems.Add , , m_objAssociationTable.GetDescription
'            lvItem.ListSubItems.Add , , m_objAssociationTable.GetBuildingHouseName
'            lvItem.ListSubItems.Add , , m_objAssociationTable.GetBuildingHouseNumber
'            lvItem.ListSubItems.Add , , m_objAssociationTable.GetStreet
'            lvItem.ListSubItems.Add , , m_objAssociationTable.GetPostCode
            lvItem.ListSubItems.Add , , m_objAssociationTable.GetAddress
' TW 16/01/2007 EP2_859 End
            
            TableAccess(m_objAssociationTable).MoveNext
        Next
' TW 16/01/2007 EP2_859
    Else
...
Private Sub Form_Unload(Cancel As Integer)
    Set m_objAssociationTable = Nothing
End Sub
...
' Description   : Loads frmEditAssociation passing through the GUID
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Private Sub EditAssociation()
