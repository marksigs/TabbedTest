...
Begin VB.Form frmEditInsuranceAdminFee 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "[Add/Edit] Insurance Admin Fee Sets"
   ClientHeight    =   5610
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   8280
   Icon            =   "frmEditInsuranceAdminFee.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame frameFeeSetBands 
      ...
      Top             =   585
      Width           =   7965
      Begin MSGOCX.MSGDataGrid dgInsuranceAdmitFee 
         Height          =   4065
         Left            =   90
         TabIndex        =   2
         Top             =   330
   ...
   End
End
Attribute VB_Name = "frmEditInsuranceAdminFee"
...
Private Sub Form_Load()
    ...
    Set m_clsTableAccess = New InsuranceAdminFeeBand
    dgInsuranceAdmitFee.Enabled = False
...
Private Sub DoSetUpdates()
    ...
    Dim clsProductFeeSet    As InsuranceAdminFeeSetTable
    Dim col                 As Collection
    ...
    sSet = txtSetNumber.Text
    If Len(sSet) > 0 Then
        Set clsProductFeeSet = New InsuranceAdminFeeSetTable
...
Private Sub dgProductSwitchFee_BeforeAdd(bCancel As Boolean, nCurrentRow As Integer)
    
    If nCurrentRow >= 0 Then
        bCancel = Not dgInsuranceAdmitFee.ValidateRow(nCurrentRow)
    End If
End Sub
...
Private Sub SetupDataGrid()
    ...
    Call colDataControls.Add(dgInsuranceAdmitFee)
...
Public Sub SetAddState()
    ...
    If txtSetNumber.Visible = True Then
        txtSetNumber.SetFocus
    End If
    Me.Caption = "Add Insurance Admin Fee Set"
End Sub
...
Public Sub SetEditState()
    ...
    dgInsuranceAdmitFee.Enabled = True
    CmdAction(2).Enabled = False
    Me.Caption = "Edit Insurance Admin Fee Set"
ExitHandler:
    Exit Sub
ErrorHandler:
    g_clsErrorHandling.RaiseError Err.Number, Err.DESCRIPTION
...
Private Sub SetGridFields()
    ...
    Dim clsAdminFeeTable        As InsuranceAdminFeeBand
    ...
    fields.sField = "InsuranceAdminFeeSet"
    ...
    Call dgInsuranceAdmitFee.SetColumns(colFields, "EditProductSwitchFees", "Product Switch Fees")
ExitHandler:
    '*=Cleanup here
...
Public Sub PopulateScreenFields()
    ...
    Dim clsAdminFees As InsuranceAdminFeeBand
...
Private Function ValidateScreenData() As Boolean
    ...
    If bRet Then
        bRet = dgInsuranceAdmitFee.ValidateRows()
    End If
...
Private Function ValidateSpecialRules()
    ...
    colFields.Add "InsuranceAdminFeeSet"
    ...
    lTotalCount = dgInsuranceAdmitFee.Rows
    '*=[MC]Iterate through all records in the datagrid and validate the data
    For lLoop = 0 To lTotalCount - 1
        Set colValues = New Collection
        sTypeofApp = dgInsuranceAdmitFee.GetAtRowCol(CInt(lLoop), 3)
        sLocation = dgInsuranceAdmitFee.GetAtRowCol(CInt(lLoop), 4)
        'Add field Values
...
Public Sub SetDataGridState()
    ...
    If bRet = True Then
        ...
        bEnabled = dgInsuranceAdmitFee.Enabled
        dgInsuranceAdmitFee.Enabled = True
        ...
        If m_bIsEdit = False Then
            If bEnabled = False Then
                ...
                dgInsuranceAdmitFee.AddRow
                dgInsuranceAdmitFee.SetFocus
            End If
        End If
    End If
