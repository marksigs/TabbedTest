...
Begin VB.Form frmEditCreditLimitIncreaseFees 
   Caption         =   "Credit Limit Increase Fee Set Add/Edit"
   ClientHeight    =   5655
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8505
   Icon            =   "frmEditCreditLimitIncreaseFees.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5655
   ...
   Begin VB.Frame frameFeeSetBands 
      ...
      Top             =   720
      Width           =   8235
      Begin MSGOCX.MSGDataGrid dgCreditLimitIncreaseFees 
         Height          =   3855
         Left            =   600
         TabIndex        =   2
         Top             =   300
   ...
   Begin VB.CommandButton cmdOK 
      ...
      Width           =   1215
   End
   Begin MSGOCX.MSGEditBox txtCreditLimitIncreaseFees 
      Height          =   315
      Index           =   0
      Left            =   1560
      TabIndex        =   0
   ...
   End
End
Attribute VB_Name = "frmEditCreditLimitIncreaseFees"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Form          :   frmEditCreditLimitIncreaseFees
' Description   :   Form which allows the user to edit Credit Limit Increase Fees. This is a banded
...
Private Sub Form_Load()
    ...
    Set m_clsTableAccess = New CreditLimitIncreaseFeeTable
    
    dgCreditLimitIncreaseFees.Enabled = False
    ...
    m_sOriginalSet = txtCreditLimitIncreaseFees(BANDED_FEE_SET).Text
    m_sOriginalStartDate = txtCreditLimitIncreaseFees(BANDED_DATE).Text
    Exit Sub
Failed:
    g_clsErrorHandling.DisplayError
...
Private Function DoesRecordExist() As Boolean
    ...
    sSet = txtCreditLimitIncreaseFees(BANDED_FEE_SET).Text
    sStartDate = txtCreditLimitIncreaseFees(BANDED_DATE).Text
    ...
    If Len(sSet) > 0 And Len(sStartDate) > 0 Then
        ...
        If bRet = True Then
            ...
            txtCreditLimitIncreaseFees(BANDED_FEE_SET).SetFocus
        End If
    Else
...
Private Sub SaveChangeRequest()
    ...
    sSet = Me.txtCreditLimitIncreaseFees(BANDED_FEE_SET).Text
    sDate = Me.txtCreditLimitIncreaseFees(BANDED_DATE).Text
...
Private Sub DoUpdates()
    ...
    sStartDate = txtCreditLimitIncreaseFees(BANDED_DATE).Text
    sSetID = txtCreditLimitIncreaseFees(BANDED_FEE_SET).Text
    ...
    If Len(sStartDate) > 0 And Len(sSetID) > 0 Then
        ...
        End If
    Else
        g_clsErrorHandling.RaiseError errGeneralError, "CreditLimitIncreaseFees - StartDate and SetID must be valid"
    End If
...
Private Sub DoSetUpdates()
    ...
    Dim clsCreditLimitFeeSet As CreditLimitIncFeeSetTable
    ...
    If Len(sSet) > 0 Then
        Set clsCreditLimitFeeSet = New CreditLimitIncFeeSetTable
        Set clsTableAccess = clsCreditLimitFeeSet
        ...
        If bRet = False Then
            ...
            clsCreditLimitFeeSet.SetFeeSet sSet
...
Private Sub dgCreditLimitIncreaseFees_BeforeAdd(bCancel As Boolean, nCurrentRow As Integer)
    
    If nCurrentRow >= 0 Then
        bCancel = Not dgCreditLimitIncreaseFees.ValidateRow(nCurrentRow)
    End If
End Sub
...
Private Sub SetupDataGrid()
    ...
    colDataControls.Add dgCreditLimitIncreaseFees
...
Public Sub SetAddState()
    ...
    If txtCreditLimitIncreaseFees(BANDED_FEE_SET).Visible = True Then
        txtCreditLimitIncreaseFees(BANDED_FEE_SET).SetFocus
    End If
End Sub
...
Public Sub SetEditState()
    ...
    txtCreditLimitIncreaseFees(BANDED_FEE_SET).Enabled = False
    txtCreditLimitIncreaseFees(BANDED_DATE).Enabled = False
    
    dgCreditLimitIncreaseFees.Enabled = True
...
Private Sub SetGridFields()
    ...
    Dim clsAdminFeeTable As CreditLimitIncreaseFeeTable
    ...
    fields.sField = "CreditLimitIncreaseFeeSet"
    fields.bRequired = True
    fields.bVisible = False
    fields.sDefault = txtCreditLimitIncreaseFees(BANDED_FEE_SET).Text
    fields.sError = ""
    ...
    fields.sField = clsAdminFeeTable.GetStartDateField() '"StartDate"
    fields.bRequired = True
    fields.bVisible = False
    fields.sDefault = txtCreditLimitIncreaseFees(BANDED_DATE).Text
    fields.sError = ""
...
' TW 14/12/2006 EP2_107 End
    ...
    dgCreditLimitIncreaseFees.SetColumns colFields, "EditCreditLimitFees", "Credit Limit Fees"
    Exit Sub
Failed:
    g_clsErrorHandling.RaiseError Err.Number, Err.DESCRIPTION
End Sub
...
Public Sub PopulateScreenFields()
    On Error GoTo Failed
    Dim clsCreditLimitIncreaseFees As CreditLimitIncreaseFeeTable
    
    Set clsCreditLimitIncreaseFees = m_clsTableAccess
    txtCreditLimitIncreaseFees(BANDED_FEE_SET).Text = clsCreditLimitIncreaseFees.GetFeeSet()
    txtCreditLimitIncreaseFees(BANDED_DATE).Text = clsCreditLimitIncreaseFees.GetStartDate()
...
Private Function ValidateScreenData() As Boolean
    ...
    If bRet Then
        bRet = dgCreditLimitIncreaseFees.ValidateRows()
    End If
...
Private Sub txtCreditLimitIncreaseFees_Validate(Index As Integer, Cancel As Boolean)
    Cancel = Not txtCreditLimitIncreaseFees(Index).ValidateData()
...
Public Sub SetDataGridState()
    ...
    If bRet = True Then
        ...
        bEnabled = dgCreditLimitIncreaseFees.Enabled
        dgCreditLimitIncreaseFees.Enabled = True
        ...
        If m_bIsEdit = False Then
            If bEnabled = False Then
                ...
                dgCreditLimitIncreaseFees.AddRow
                dgCreditLimitIncreaseFees.SetFocus
            End If
        End If
    End If
    Exit Sub
