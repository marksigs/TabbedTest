...
Begin VB.Form frmEditProductSwitchFee 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "[Add/Edit] Product Switching Fee Set "
   ClientHeight    =   5640
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   8310
   ...
   EndProperty
   Icon            =   "frmEditProductSwitchFee.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame frameFeeSetBands 
      ...
      Top             =   630
      Width           =   7965
      Begin MSGOCX.MSGDataGrid dgProductSwitchFee 
         Height          =   4065
         Left            =   90
         TabIndex        =   2
         Top             =   330
   ...
   End
End
Attribute VB_Name = "frmEditProductSwitchFee"
...
Private Sub Form_Load()
    ...
    Set m_clsTableAccess = New ProductSwitchFeeTable 'AdminFeeTable
    dgProductSwitchFee.Enabled = False
...
Private Function ValidateSpecialRules()
    ...
    sStartDate = txtStartDate.Text
    lTotalCount = dgProductSwitchFee.Rows
    '*=[MC]Iterate through all records in the datagrid and validate the data
    For lLoop = 0 To lTotalCount - 1
        Set colValues = New Collection
        sTypeofApp = dgProductSwitchFee.GetAtRowCol(CInt(lLoop), 3)
        sLocation = dgProductSwitchFee.GetAtRowCol(CInt(lLoop), 4)
        'Add field Values
...
Private Sub DoSetUpdates()
    ...
    Dim clsProductFeeSet    As ProductSwitchFeeSetTable
    Dim col                 As Collection
    ...
    sSet = txtSetNumber.Text
    If Len(sSet) > 0 Then
        Set clsProductFeeSet = New ProductSwitchFeeSetTable
...
Private Sub dgProductSwitchFee_BeforeAdd(bCancel As Boolean, nCurrentRow As Integer)
    ...
    If nCurrentRow >= 0 Then
        bCancel = Not dgProductSwitchFee.ValidateRow(nCurrentRow)
    End If
End Sub
...
Private Sub SetupDataGrid()
    ...
    Call colDataControls.Add(dgProductSwitchFee)
...
Public Sub SetAddState()
    ...
    If txtSetNumber.Visible = True Then
        txtSetNumber.SetFocus
    End If
    Me.Caption = "Add Product Switching Fee Set"
End Sub
...
Public Sub SetEditState()
    ...
    dgProductSwitchFee.Enabled = True
    CmdAction(2).Enabled = False
    Me.Caption = "Edit Product Switching Fee Set"
ExitHandler:
    Exit Sub
ErrorHandler:
    g_clsErrorHandling.RaiseError Err.Number, Err.DESCRIPTION
...
Private Sub SetGridFields()
    ...
    Dim clsAdminFeeTable        As ProductSwitchFeeTable
    ...
    fields.sField = "ProductSwitchFeeSet"
    ...
    Call dgProductSwitchFee.SetColumns(colFields, "EditProductSwitchFees", "Product Switch Fees")
ExitHandler:
    '*=Cleanup here
    Set colFields = Nothing
...
Public Sub PopulateScreenFields()
    ...
    Dim clsAdminFees As ProductSwitchFeeTable
...
Private Function ValidateScreenData() As Boolean
    ...
    If bRet Then
        bRet = dgProductSwitchFee.ValidateRows()
    End If
...
Public Sub SetDataGridState()
    ...
    If bRet = True Then
        ...
        bEnabled = dgProductSwitchFee.Enabled
        dgProductSwitchFee.Enabled = True
        ...
        If m_bIsEdit = False Then
            If bEnabled = False Then
                ...
                dgProductSwitchFee.AddRow
                dgProductSwitchFee.SetFocus
            End If
        End If
    End If
