//------------------------------------------------------------------------------
// SR   25/10/2006      EP2_11 - new 
// SR	05/02/2007		EP2_1163 - modified omInterface- added functionality to validate the format of password
// SR	21/02/2007		EP2_1355 -
//------------------------------------------------------------------------------
using System;
...
namespace Vertex.Fsd.Omiga.Web.Services.BrokerRegAndMaintainWS
{
    ...
    public class XmlStreamSoapExtension : SoapExtension
    {
		...
		private void om4Interface(XmlDocument soapOutXDoc)
		{
			...
			XmlNode omigaResponseNode ;
			// SR EP2_1163 : DO Password validation
			int message = 0;
			bool pwdValid = true;
			bool raiseError = false ;
			string soapAction = HttpContext.Current.Request.Headers.Get("SOAPAction");
			string wsMethod = "";
			if(soapAction.IndexOf("BrokerRegistration") > 0) wsMethod = "BrokerRegistration";
			else if(soapAction.IndexOf("UpdateBrokerProfile") > 0 ) wsMethod = "UpdateBrokerProfile";
			if(wsMethod == "BrokerRegistration" || wsMethod == "UpdateBrokerProfile" )
			{
				NameTable nt = new NameTable();
				XmlNamespaceManager nsmgr = new XmlNamespaceManager(nt);
				nsmgr.AddNamespace("x", "http://Request.BrokerRegAndMaintain.Omiga.vertex.co.uk");
				nsmgr.AddNamespace("intrprof", "http://IntroducerDetail.Omiga.vertex.co.uk");
				if(wsMethod == "BrokerRegistration" || xReq.SelectSingleNode("//intrprof:PASSWORD[@CRUD_OP='CREATE']",nsmgr) != null)
				{
					string userId;
					string password;
					string emailAddress;
				
					omIntroducer.Introducer introducer = new omIntroducer.Introducer();	
					if(wsMethod == "BrokerRegistration")
					{
						password = xReq.SelectSingleNode("//intrprof:PASSWORD/@PASSWORDVALUE", nsmgr).Value;
						emailAddress = xReq.SelectSingleNode("//x:INTRODUCER/@EMAILADDRESS", nsmgr).Value;
						pwdValid = introducer.ValidatePasswordFormat(password, emailAddress, out message);
					}
					else
					{
						password = xReq.SelectSingleNode("//intrprof:PASSWORD[@CRUD_OP='CREATE']/@PASSWORDVALUE", nsmgr).Value; //EP2_1355
						userId =  xReq.SelectSingleNode("//x:INTRODUCER/@USERID", nsmgr).Value;
						emailAddress = xReq.SelectSingleNode("//x:INTRODUCER/@EMAILADDRESS", nsmgr).Value;
						pwdValid = introducer.ValidatePasswordFormat(password, emailAddress, out message, userId);
					}
				}				
			
				if(!pwdValid && message != 0) 
				{
					raiseError = true ;
					OmigaException omEx = new OmigaException(message);
					string errResponse = omEx.ToOmiga4Response();					
					responseDoc.LoadXml(errResponse);					
					AddErrorNamespaces(responseDoc);
				}				
			}
			
			if(!raiseError) // SR EP2_1163 - End
			{
				string WSInterfaceMethod ;
				WSInterfaceMethod = xReq.DocumentElement.Attributes.GetNamedItem("WSINTERFACEMETHOD").Value;

				Type lbt = Type.GetTypeFromProgID("omWSInterface.omWSInterfaceBO");
				object lbo = Activator.CreateInstance(lbt);
				object[] lbop = new object[] {WSInterfaceMethod, sx};
			...
			if((wsMethod == "BrokerRegistration" || wsMethod == "UpdateBrokerProfile" ) && raiseError)
					omigaResponseNode = responseDoc.DocumentElement; 
			else omigaResponseNode = xd.DocumentElement;			
		...
		// SR EP2_1163 : new function AddErrorNamespaces
