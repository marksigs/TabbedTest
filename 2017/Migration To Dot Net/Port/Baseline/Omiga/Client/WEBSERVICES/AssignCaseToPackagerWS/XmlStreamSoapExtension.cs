...
namespace Vertex.Fsd.Omiga.Web.Services.AssignCaseToPackagerWS
{
    ...
    public class XmlStreamSoapExtension : SoapExtension
    {
		...
		private void om4Interface(XmlDocument soapOutXDoc)
		{
			...
			object[] lbop = new object[] {"AssignCaseToPackager", sx};

			XmlDocument xd = new XmlDocument();

			if (_logger.IsDebugEnabled)
			{
				_logger.DebugFormat("{0}: Calling omWSInterface with request:{1}.", functionName, sx);
			}
			...
			if (_logger.IsDebugEnabled)
			{
				_logger.DebugFormat("{0}: Returned from omWSInterface with response:{1}.", functionName, xd.OuterXml);
			}
