...
'PSC    19/09/2002  SYS4752     Applied this core change to BMids version'
'MV     03/03/2003  BM0378      Amended adoCreate() to support ROWLOCK
'-------------------------------------------------------------------------------
...
Public Sub adoCreate(ByVal vxmlDataNode As IXMLDOMNode, ByVal vxmlSchemaNode As IXMLDOMNode)
    ...
    strSQL = "INSERT INTO " & strDataSource
    
    'MV - 03/03/2003 - BM0378
    'Start
    If (genumDbProvider = omiga4DBPROVIDERSQLServer) And (xmlGetAttributeText(vxmlSchemaNode, "SQLROWLOCK") = "TRUE") Then
        strSQL = strSQL & " WITH (ROWLOCK) "
    End If
    'End
    
    strSQL = strSQL & " (" & strColumns & " ) "
...
On Error GoTo adoGetRecordSetExit
    ...
    Dim blnDbCmdOk As Boolean
    Dim strSQLNoLock As String      ' PSC 19/09/2002 SYS4752
    ...
    Else
        ...
        If cmd.Parameters.Count <> 0 Then
            ...
            ' Allow schema to specify the SQL-Server (NOLOCK) hint.
...
Public Function executeGetRecordSetEx( _
    ByVal cmd As ADODB.Command) _
    As ADODB.Recordset
    
On Error GoTo executeGetRecordSetExit
    
    Const strFunctionName As String = "executeGetRecordSetEx"
    ...
    Set rstReturn = rst.NextRecordset
    If rstReturn Is Nothing Then
        Set rstReturn = rst
    End If
    
    If Not rstReturn Is Nothing Then
        If Not rstReturn.EOF Then
            rstReturn.MoveFirst
        End If
    End If
    
    Set executeGetRecordSetEx = rstReturn
    'disconnect RecordSet
    Set rstReturn.ActiveConnection = Nothing
    conn.Close
