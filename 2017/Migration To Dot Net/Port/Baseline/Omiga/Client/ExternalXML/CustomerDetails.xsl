	    ...
	    function SetSalutation(Title,Surname,Counter)
	    {
			if (Counter == '1') 
			{
				if (Title !='')
					...
					Applicant1Title=Applicant1Title + Title;
				if (Surname !='')
		  ...
		  else if (Counter == '2') 
			{
				if (Title !='')
					...
					Applicant2Title=Applicant2Title + Title;
				if (Surname !='')
					titlename=titlename + ' ' + Surname ;
   ...
   return(TenType);
	...
	<xsl:template match="/">
		<xsl:element name="TEMPLATEDATA">
			<xsl:variable name="CustCount" select="count(RESPONSE/CUSTOMERDETAILS/UNIQUEAPPLICATIONADDRESSES)"/>
			<xsl:variable name="CustRole" select="count(RESPONSE/CUSTOMERDETAILS/CUSTOMERROLE)"/>
				...
				<xsl:if test="($CustRole = '2')">
						<xsl:for-each select="//CUSTOMERROLE">
							...
							<xsl:if test="($Order_No = '1')">
									...
									<xsl:variable name="ApplicantTitles" select="msg:GetApplicantNameWithInitials(string(.//CUSTOMERVERSION/@TITLE_TEXT),
						...
						<xsl:element name="INFORMATION">
								<xsl:element name="CUSTOMERDETAILS">
									...
									<xsl:attribute name="APPLICANT1TITLE"><xsl:value-of select="msg:GetApplicant1Title()"/></xsl:attribute>
									<xsl:attribute name="APPLICANT2TITLE"><xsl:value-of select="msg:GetApplicant2Title()"/></xsl:attribute>																		
									<xsl:variable name="TenureType" select="msg:GetTenure()"/>
									<xsl:choose>
										<xsl:when test="($TenureType != '1')">
											<xsl:element name="TENURETYPE"/>									
										</xsl:when>
									</xsl:choose>
				...
				<xsl:for-each select="//CUSTOMERROLE">
					<xsl:element name="INFORMATION">
							<xsl:element name="CUSTOMERDETAILS">
								...
								<xsl:attribute name="APPLICANT1TITLE"><xsl:value-of select="msg:GetApplicant1Title()"/></xsl:attribute>
								<xsl:attribute name="APPLICANT2TITLE"><xsl:value-of select="msg:GetApplicant2Title()"/></xsl:attribute>
								<xsl:variable name="TenureType1" select="msg:SetTenureType(.//NEWPROPERTY/@TENURETYPE)"/>
								<xsl:variable name="TenureType" select="msg:GetTenureType()"/>								
								<xsl:attribute name="TenureType"><xsl:value-of select="$TenureType"/></xsl:attribute>									
