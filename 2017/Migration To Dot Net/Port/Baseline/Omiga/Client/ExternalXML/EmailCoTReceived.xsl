	...
	<xsl:template match="/">
		<xsl:element name="TEMPLATEDATA">
			<xsl:element name="APPLICATION">
				...
				<xsl:attribute name="APPLICANTNAME"><xsl:value-of select="/RESPONSE/APPLICATION/@CORRESPONDENCESALUTATION"/></xsl:attribute>
			</xsl:element>
			<xsl:element name="ADVANCE">
				<xsl:attribute name="ADVANCEDATE"><xsl:value-of select="RESPONSE/ADVANCE/@ADVANCEDATE"/></xsl:attribute>
			</xsl:element>
		</xsl:element>
	</xsl:template>
