...
<!--DRC  Modified for use with pack 09/01/2007 EP1274 -->
...
function GetTenure()
{
   return(TenType);
    ...
    <xsl:variable name="ApplicantTitles"/>    
     <xsl:variable name="ApplicantCount"/>   
		...
		<xsl:element name="TEMPLATEDATA">
						...
						<xsl:for-each select=".//CUSTOMER">
							...
							<xsl:if test="($Order_No = '1')">								       
							        <xsl:variable name="SALUTATIONONE" select="msg:SetSalutation('', string(.//@TITLE) ,string(.//@OTHERTITLE), string(.//@FIRSTFORENAME),string(.//@SURNAME), 1)"/>
							        <xsl:variable name="CustomerNo" select="msg:SetCustomerNo(concat('{',.//@CUSTOMERNUMBER,'}'),1)"/>
							 </xsl:if>		
							<xsl:if test="($Order_No = '2')">
							       <xsl:variable name="SALUTATIONONE" select="msg:SetSalutation('', string(.//@TITLE) ,string(.//@OTHERTITLE), string(.//@FIRSTFORENAME),string(.//@SURNAME), 2)"/>
							       <xsl:variable name="CustomerNo" select="msg:SetCustomerNo(concat('{',.//@CUSTOMERNUMBER,'}'),2)"/>
							  </xsl:if>      
						     <xsl:variable name="AddressName" select="msg:GetApplicantNameWithInitials('', string(.//@TITLE), '', string(.//@FIRSTFORENAME), '',  string(.//@SURNAME),	1)"/> 
						</xsl:for-each>	
						<xsl:element name="INFORMATION">
								...
								<xsl:variable name="ApplicantCount" select="msg:GetCounter(1)"/>				
								<xsl:if test="($ApplicantCount = 1)">				
                                     <xsl:attribute name="CURRENTADDRESS"><xsl:value-of select="msg:GetAddress(
											string(.//CUSTOMER/@TITLE_TYPES), 
											string(.//CUSTOMER/@TITLE), 
											string(.//CUSTOMER/@TITLEOTHER), 
											string(.//CUSTOMER/@FIRSTFORENAME),
											string(.//CUSTOMER/@SURNAME),												
												...
												string(.//TEMPLATEDATA/ADDRESS/@POSTCODE))"/></xsl:attribute>
									<xsl:attribute name="COAPPLICANT"><xsl:value-of select="msg:GetBothNames(
											string(''), 
											string(.//CUSTOMER/@TITLE), 
											string(.//CUSTOMER/@TITLEOTHER), 
											string(.//CUSTOMER/@SURNAME))"/></xsl:attribute>
									<xsl:attribute name="CUSTOMERNUMBER"><xsl:value-of select="string(.//CUSTOMER/@CUSTOMERNUMBER)"/></xsl:attribute>
									...
									<xsl:attribute name="APPLICATIONNUMBER"><xsl:value-of select="concat('{',.//ADDRESS/@APPLICATIONNUMBER,'}')"/></xsl:attribute>
