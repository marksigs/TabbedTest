...
function DealWithEmployerAddress(CompanyName,Title,FirstName,Surname,FlatNumber, HouseName, HouseNumber, Street, District, Town, County, PostCode)
        {
	var strAddress = "";
			...
			<xsl:for-each select="//CUSTOMERROLE">
				<xsl:element name="INFORMATION">
					<xsl:element name="CUSTOMERDETAILS">
						<xsl:attribute name="EMPLOYERADDRESS"><xsl:value-of select="msg:DealWithEmployerAddress(
																											  ...
																											  string(.//THIRDPARTY/ADDRESS/@POSTCODE))"/></xsl:attribute>
						...
						<xsl:attribute name="SEX"><xsl:value-of select="msg:GetSexDescription(string(.//COMBOVALUE/@VALUEID))"/></xsl:attribute>
