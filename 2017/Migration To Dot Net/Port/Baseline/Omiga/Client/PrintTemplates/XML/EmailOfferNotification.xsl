	...
	<xsl:template match="/">
		<xsl:element name="TEMPLATEDATA">
			<xsl:element name="APPLICATION">
				<xsl:attribute name="APPLICATIONNUMBER"><xsl:value-of select="concat('{', /RESPONSE/EMAILOFFERNOTIFICATION/@APPLICATIONNUMBER, '}')"/></xsl:attribute>
				<xsl:attribute name="APPLICANTNAME"><xsl:value-of select="/RESPONSE/EMAILOFFERNOTIFICATION/@CORRESPONDENCESALUTATION"/></xsl:attribute>
			</xsl:element>
		</xsl:element>
	</xsl:template>
