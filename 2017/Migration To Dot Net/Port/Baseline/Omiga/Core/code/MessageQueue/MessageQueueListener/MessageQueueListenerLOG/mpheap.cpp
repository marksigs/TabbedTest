...
Abstract: 
    ...
    a significant performance penalty when running on a multi-processor machine.  
	This is due to the serialization used by the default heap package.  On a 
	multiprocessor machine, more than one thread may simultaneously try to allocate 
	memory.  One thread will block on the critical section guarding the heap.  
	The other thread must then signal the critical section when it is finished to 
	unblock the waiting thread.  The additional codepath of blocking and signalling 
	adds significant overhead to the frequent memory allocation path. 
...
{ 
    ...
    MpHeap = (_MP_HEAP*)HeapAlloc(Heap,0,sizeof(MP_HEAP) + 
    ...
    MpHeap->Entry[0].Allocations = 0; 
    MpHeap->Entry[0].Frees = 0; 
    MpHeap->Entry[0].LookasideAllocations = 0; 
    MpHeap->Entry[0].LookasideFrees = 0; 
    MpHeap->Entry[0].DelayedFrees = 0; 
    ...
    for (i=1; i<dwParallelism; i++) { 
		...
		MpHeap->Entry[i].Allocations = 0; 
		MpHeap->Entry[i].Frees = 0; 
		MpHeap->Entry[i].LookasideAllocations = 0; 
		MpHeap->Entry[i].LookasideFrees = 0; 
		MpHeap->Entry[i].DelayedFrees = 0; 
...
LockAcquired: 
    ...
    Header = (_MP_HEADER*)HeapAlloc(Entry->Heap, 0, Size); 
    LeaveCriticalSection(&Entry->Lock); 
    if (Header != NULL) { 
...
{ 
    ...
    Header = (_MP_HEADER*)HeapReAlloc(Header->HeapEntry->Heap, 0, Header, dwBytes); 
...
{ 
    ...
    do { 
        ...
        Header->Next = Next; 
    } while ( InterlockedCompareExchange((void**)&HeapEntry->DelayedFreeList, 
                                         &Header->Next, 
                                         Next) != Next); 
    return(TRUE); 
} 
