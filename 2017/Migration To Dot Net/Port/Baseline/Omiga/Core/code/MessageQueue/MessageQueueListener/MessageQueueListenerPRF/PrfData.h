...
public:     // Static member functions for install/uninstall
   ...
   static void Install(LPCWSTR pszDllPathname, LPCWSTR pszAppName = NULL);
...
public:     // Static member function to activate counter information
   ...
   static DWORD Activate(LPCWSTR pszAppName = NULL); // Returns last error
...
public:  // Public member functions
   ...
   // Removes all instances from an object
   void RemoveAllInstances(OBJID ObjId);
...
private: // Fuctions for install/remove
   ...
   void     InstallPrfData(LPCWSTR pszDllPathname, LPCWSTR pszAppName = NULL);
...
private: // Functions to convert betwen value types
   DWORD   ActivatePrfData(LPCWSTR pszAppName); // Returns last error
...
inline void CPrfData::Install(LPCWSTR pszDllPathname, LPCWSTR pszAppName) {
   sm_pPrfData->InstallPrfData(pszDllPathname, pszAppName);
}
...
inline DWORD CPrfData::Activate(LPCWSTR pszAppName) { 
   return(sm_pPrfData->ActivatePrfData(pszAppName));
}
...
#define PRFDATA_MAP_OBJ(dwId, pszName, pszHelp, dwDetailLevel,          \
   ...
   { CPrfData::PIT_OBJECT, (DWORD) dwId, pszName, pszHelp,    \
...
#define PRFDATA_MAP_CTR(dwId, pszName, pszHelp, dwDetailLevel,          \
   dwDefScale, dwType)                                                  \
   { CPrfData::PIT_COUNTER, (DWORD) dwId, pszName, pszHelp,   \
