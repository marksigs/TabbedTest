...
void CPrfData::InstallPrfData(LPCWSTR pszDllPathname, LPCWSTR pszAppName) {
   ...
   if (
		ERROR_SUCCESS == regApp.OpenSubkey(FALSE, HKEY_LOCAL_MACHINE, szSubkey) && 
		ERROR_SUCCESS == regApp.GetDWORD(L"First Counter", &dwFirstCounter) &&
		dwFirstCounter > 0)
   {	
		// Counter/help app values already installed, so remove them first.
		UninstallPrfData();
   }

   // Read the last counter/help global values from the registry
   chVERIFY(ERROR_SUCCESS == 
      regPerfLib.OpenSubkey(FALSE, HKEY_LOCAL_MACHINE, 
   ...
   wsprintfW(szSubkey, 
      ...
      m_pszAppName);
   chVERIFY(ERROR_SUCCESS == 
      regApp.OpenSubkey(FALSE, HKEY_LOCAL_MACHINE, szSubkey));
...
void CPrfData::AppendRegStrings(CRegSettings& regPerfLib, 
   ...
   CRegSettings regLang(FALSE, regPerfLib, L"009");
...
DWORD CPrfData::ActivatePrfData(LPCWSTR pszAppName) 
{
	if (pszAppName)
	{
		m_pszAppName = pszAppName;
		  ...
		  (m_dwLastHelp - m_dwFirstHelp) / 2 + 1)
   {
	   ...
	   m_hfm = CreateFileMappingW((HANDLE)MMF_PAGING_FILE, lpsa, PAGE_READWRITE, 
	   ...
	   m_pPrfData = (PBYTE) MapViewOfFile(m_hfm, FILE_MAP_ALL_ACCESS, 0, 0, 0);
	...
	{
		::SetLastError(dwErr = ERROR_INVALID_DATA);
	}
...
/////////////////////////////////////////////////////////////////////
// AS 02/13/01 Removes all instances from an object.
void CPrfData::RemoveAllInstances(OBJID ObjId)
{
   OBJORD ObjOrd = CvtObjIdToObjOrd(ObjId);
   chASSERT(IsValidObjOrd(ObjOrd));
   PRFMETRICS pm;
   ...
   for (; InstId < (INSTID) pm.MaxInstances; InstId++) {
      CalcPrfMetrics(ObjOrd, InstId, &pm);
      BOOL bRemove = pm.pPIN[0] != 0;
	  if (bRemove)
	  {
		  RemoveInstance(ObjId, InstId);
	  }
   }
}
...
void CPrfData::CalcPrfMetrics(OBJORD ObjOrd, INSTID InstId, 
   ...
   pPM->cbPCD = (int) pPrfItm->NumCounters * 
      sizeof(PERF_COUNTER_DEFINITION);
...
DWORD CPrfData::CollectAnObj(OBJORD ObjOrd, PBYTE *ppbData) const {
   ...
   CalcPrfMetrics(ObjOrd, 0, &pm);
	...
	// AS 31/08/00 From personal e-mail from Jeffrey Richter to make PERF_ELAPSED_TIME work.
	FILETIME startTimeAsFileTime;
	GetSystemTimeAsFileTime(&startTimeAsFileTime);
	CopyMemory(&pm.pPOT->PerfTime, &startTimeAsFileTime, sizeof(startTimeAsFileTime));
	pm.pPOT->PerfFreq.QuadPart = 10000000; // convert 100 nanoseconds to seconds in the function 
	// AS 31/08/00 End
