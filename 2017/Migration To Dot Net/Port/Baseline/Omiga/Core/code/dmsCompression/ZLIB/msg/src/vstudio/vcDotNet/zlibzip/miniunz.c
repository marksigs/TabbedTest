	...
	long* plUnzippedFilesCount;
{
	...
	if (plUnzippedFilesCount != NULL && pUnzippedFiles != NULL && plUnzippedFilesMaxSize != NULL)
	{
		...
		lUnzippedFilesMaxSize = (pUnzippedFilesTemp - pUnzippedFiles) + strlen(filename_inzip) + 1;
    ...
    else
    {
        ...
        if (fout!=NULL || pUnzippedFiles!=NULL)
        {
			...
			{
				lUnzippedFilesMaxSize = (pUnzippedFilesTemp - pUnzippedFiles) + (long)sizeof(lUnzippedFileSize);
				if (lUnzippedFilesMaxSize > *plUnzippedFilesMaxSize)
				{
					errMem = UNZ_MOREMEM;
				}
            ...
            do
            {
                ...
                if (err>0)
				{
					if (pUnzippedFiles != NULL && plUnzippedFilesMaxSize != NULL)
					{
						...
						lUnzippedFilesMaxSize = (pUnzippedFilesTemp - pUnzippedFiles) + err;
						if (errMem == UNZ_OK)
						{
							if (lUnzippedFilesMaxSize > *plUnzippedFilesMaxSize)
							{
	...
	if (err == UNZ_OK)
	{
		if (pUnzippedFiles != NULL && pUnzippedFilesTemp != NULL && plUnzippedFilesMaxSize != NULL)
		{
			...
			*plUnzippedFilesMaxSize = pUnzippedFilesTemp - pUnzippedFiles;
		}
