...
'
'Prog   Date        AQR         Description
'GHun   19/01/06    MAR1085 Improved error message in GetNode
'GHun   13/03/2006  MAR1300 Merged core changes
'------------------------------------------------------------------------------------------
'EPSOM History
'
'Prog   Date        AQR         Description
'PB     13/07/2006  EP992       'Object not set' error - need to check it first
'
'------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
    ...
    Dim lngListLength As Long
    lngListLength = vxmlNodeList.length - 1
    For lngNodeCount = 0 To lngListLength
...
'------------------------------------------------------------------------------------------
    ...
    Const strFunctionName As String = "xmlLoad"
...
'------------------------------------------------------------------------------------------
    ...
    Const cstrFunctionName As String = "GetNode"

    Dim xmlNode As IXMLDOMNode

    If vxmlParentNode Is Nothing Then
        errThrowError cstrFunctionName, oeMissingElement, "Missing parent node. Match pattern: " & vstrMatchPattern    'MAR1085 GHun
    End If
...
'------------------------------------------------------------------------------------------
    ...
    Const cstrFunctionName As String = "xmlGetMandatoryNodeList"
...
'PB 13/07/2006 EP992 Check that VXMLNode is valid first
...
On Error GoTo xmlChangeNodeNameVbErr
    ...
    Const strFunctionName As String = "xmlChangeNodeName"
...
'------------------------------------------------------------------------------------------
    ...
    Const strFunctionName As String = "xmlCreateElementRequestFromNode"
    ...
    If Not vxmlNode Is Nothing Then
        ...
        intListLength = vxmlNode.Attributes.length - 1
        For intNodeCount = 0 To intListLength
            xmlCopyAttribute vxmlNode, xmlRequestNode, vxmlNode.Attributes(intNodeCount).nodeName
        Next
...
'------------------------------------------------------------------------------------------
    ...
    Dim intAttribsLength As Integer
    Dim intAttribsCount As Integer
...
Const strFunctionName As String = "xmlMakeNodeElementBased"
    ...
    If Not vxmlNode Is Nothing Then
        ...
        Else
            ...
            For intAttribsCount = 0 To intAttribsLength                         '++ SA SYS4754
                'SR 12/06/01 : if node name is description of any combo value, create a new attribute
                '              to the respective node
                strAttribName = vxmlNode.Attributes(intAttribsCount).nodeName
                If Right$(strAttribName, 5) = "_TEXT" Then
                    strComboValueNodeName = Left$(strAttribName, Len(strAttribName) - 5)
                    Set xmlNode2 = xmlDestParentNode.selectSingleNode("./" & strComboValueNodeName)
                    If Not xmlNode2 Is Nothing Then
                        xmlSetAttributeValue xmlNode2, "TEXT", vxmlNode.Attributes(intAttribsCount).Text
                    Else
                        Set xmlNode = xmlDOMDocument.createElement(vxmlNode.Attributes(intAttribsCount).nodeName)    '++ SA SYS4754
                        If Not xmlNode Is Nothing Then
                            xmlNode.Text = vxmlNode.Attributes(intAttribsCount).Text
                            xmlDestParentNode.appendChild xmlNode
                        End If
                    End If
                Else
                    Set xmlNode = xmlDOMDocument.createElement(vxmlNode.Attributes(intAttribsCount).nodeName)
                    If Not xmlNode Is Nothing Then
                        xmlNode.Text = vxmlNode.Attributes(intAttribsCount).Text
                        xmlDestParentNode.appendChild xmlNode
                    End If
...
'------------------------------------------------------------------------------------------
    ...
    If Not vxmlNode Is Nothing Then
        ...
        intChildLength = vxmlNode.childNodes.length - 1
        
        For intChildCount = 0 To intChildLength
            
            'Check if it is a 'text' node or genuinely has child elements
            If vxmlNode.childNodes(intChildCount).hasChildNodes Then
                If vxmlNode.childNodes(intChildCount).childNodes(0).nodeName = "#text" Then
                    blnNoChildren = True
                End If
            Else
                blnNoChildren = True
            ...
            If blnNoChildren And Len(Trim$(vxmlNode.childNodes(intChildCount).Text)) > 0 Then
                
                'Create as an attribute of parent
                xmlParent.setAttribute vxmlNode.childNodes(intChildCount).nodeName, vxmlNode.childNodes(intChildCount).Text
                
                'Check if combo item with TEXT attribute
                intAttribsLength = vxmlNode.childNodes(intChildCount).Attributes.length - 1
                
                For intAttribsCount = 0 To intAttribsLength
                
                    If UCase$(vxmlNode.childNodes(intChildCount).Attributes(intAttribsCount).nodeName) = "TEXT" Then
            ...
            ElseIf blnConvertChild And Not blnNoChildren Then
                ...
                Set xmlNode = xmlCreateAttributeBasedResponse(vxmlNode.childNodes(intChildCount), blnConvertChild)
...
Public Function xmlCreateChildRequest(ByVal vxmlDataNode As IXMLDOMElement, ByVal vxmlSchemaNode As IXMLDOMNode, _
    ...
    ' Raise Errors:
    '------------------------------------------------------------------------------------------
    Const strFunctionName As String = "xmlCreateChildRequest"

    Dim xmlDoc As FreeThreadedDOMDocument40
    Dim xmlNewNode As IXMLDOMElement
    Dim xmlAttrib As IXMLDOMAttribute
    
    Dim intSchemaLength As Integer
    ...
    If (Not vxmlDataNode Is Nothing) And (Not vxmlSchemaNode Is Nothing) And Len(Trim$(vstrNewName)) > 0 Then
        ...
        intSchemaLength = vxmlSchemaNode.childNodes.length - 1
        For intSchemaCount = 0 To intSchemaLength
            If Not vxmlSchemaNode.childNodes(intSchemaCount).Attributes.getNamedItem("KEYTYPE") Is Nothing Then
                strValue = vxmlSchemaNode.childNodes(intSchemaCount).Attributes.getNamedItem("KEYTYPE").Text
                If strValue = "PRIMARY" Then
                    'Get this attribute from the data element and add it to the new node
                    Set xmlAttrib = vxmlDataNode.Attributes.getNamedItem(vxmlSchemaNode.childNodes(intSchemaCount).nodeName)
                    If Not xmlAttrib Is Nothing Then
                        xmlNewNode.setAttribute vxmlSchemaNode.childNodes(intSchemaCount).nodeName, xmlAttrib.Text
                    End If
                End If
            End If
        Next
...
'                            extracted
'------------------------------------------------------------------------------------------
    Const strFunctionName As String = "AttachResponseData"
    
    Dim xmlChildList As IXMLDOMNodeList
        
    Dim intChildListLength As Integer
    Dim intChildCount As Integer
    ...
    End If
     ...
     intChildListLength = xmlChildList.length - 1
     For intChildCount = 0 To intChildListLength
        If xmlChildList(intChildCount).nodeName <> "MESSAGE" Then
...
Public Function xmlCreateDOMObject() As FreeThreadedDOMDocument40
    Set xmlCreateDOMObject = New FreeThreadedDOMDocument40
    xmlCreateDOMObject.setProperty "NewParser", True
    xmlCreateDOMObject.validateOnParse = False
    xmlCreateDOMObject.async = False
End Function
