...
'------------------------------------------------------------------------------------------
    ...
    If Not errGetDefaultMessageText(vlngMessageNo, strMessageText) Then
        ...
        Set xmlElem = xmlDoc.createElement("MESSAGE")
        xmlElem.setAttribute "DATASRCE", "MESSAGE"
        ...
        adoLegacyGetDatasrce xmlRequestNode, xmlResponseNode, xmlSchemaNode, False
...
End Sub
Public Function errIsDbError() As Boolean
' header ----------------------------------------------------------------------------------
' description:
...
'------------------------------------------------------------------------------------------
    ...
    If lngErrNo <> 0 Then
        ...
        ' not be in the ADO error number range see ADODB.ErrorValueEnum
        If (lngOmigaErrorNo > 0 And _
            (lngOmigaErrorNo >= clngADO_START_ERROR_NO And _
            lngOmigaErrorNo <= clngADO_END_ERROR_NO)) Then
            errIsDbError = True
        End If
    End If
End Function
Public Sub errAddToErrSource(ByVal vstrDescription As String)
