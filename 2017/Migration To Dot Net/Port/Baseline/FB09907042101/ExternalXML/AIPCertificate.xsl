...
function GetBothNames(Title,Firstname,Secondname,Surname)
{
    ...
    <xsl:variable name="CustRole"/>        
	<xsl:template match="/">
		<xsl:element name="TEMPLATEDATA">
			<xsl:variable name="CustCount" select="count(RESPONSE/AIPCERTIFICATE/UNIQUEAPPLICATIONADDRESSES)"/>
			<xsl:variable name="CustRole" select="count(RESPONSE/AIPCERTIFICATE/CUSTOMERROLE)"/>
				...
				<xsl:if test="($CustRole = '2')">
						<xsl:element name="INFORMATION">
								<xsl:element name="CUSTOMERDETAILS">
									<xsl:attribute name="DATE"><xsl:value-of select="msg:GetDate()"/></xsl:attribute>
