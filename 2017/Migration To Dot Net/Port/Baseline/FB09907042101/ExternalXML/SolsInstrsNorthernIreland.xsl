...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:msg="http://marlborough-stirling.com/xpath-functions" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ms="urn:schemas-microsoft-com:xslt">
	...
	<xsl:template match="/">
		<xsl:element name="TEMPLATEDATA">
			<xsl:apply-templates select="/RESPONSE/SOLSINSTRSNORTHERNIRELAND/APPLICATIONLEGALREP"/>
			<xsl:call-template name="APPLICANT">
				<xsl:with-param name="RESPONSE" select="/RESPONSE/SOLSINSTRSNORTHERNIRELAND"/>
			</xsl:call-template>
			<xsl:element name="TCWORDING">
				<xsl:attribute name="TCEDITIONYEAR">
					<xsl:value-of select="concat('{', /RESPONSE/SOLSINSTRSNORTHERNIRELAND/GLOBALPARAMETER[@NAME='TemplateTCEditionYearNIreland']/@STRING,'}')"/>
				</xsl:attribute>
			</xsl:element>
			<xsl:apply-templates select="/RESPONSE/SOLSINSTRSNORTHERNIRELAND/CUSTOMERROLE/NEWPROPERTY/COMBOVALIDATION[@VALIDATIONTYPE='L']"/>
			<xsl:apply-templates select="/RESPONSE/SOLSINSTRSNORTHERNIRELAND/OTHERRESIDENT/PERSON"/>
		</xsl:element>
	</xsl:template>
	<!--============================================================================================================-->
