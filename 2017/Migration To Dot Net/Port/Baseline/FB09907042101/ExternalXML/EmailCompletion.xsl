	...
	<xsl:template match="/">
		<xsl:element name="TEMPLATEDATA">
			<xsl:element name="APPLICATION">
				<xsl:attribute name="APPLICATIONNUMBER"><xsl:value-of select="concat('{', /RESPONSE/EMAILCOMPLETION/@APPLICATIONNUMBER, '}')"/></xsl:attribute>
				<xsl:attribute name="APPLICANTNAME"><xsl:value-of select="/RESPONSE/EMAILCOMPLETION/@CORRESPONDENCESALUTATION"/></xsl:attribute>
			</xsl:element>
		</xsl:element>
	</xsl:template>
