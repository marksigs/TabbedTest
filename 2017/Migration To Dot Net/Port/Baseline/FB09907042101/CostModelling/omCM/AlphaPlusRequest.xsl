			...
			<xsl:element name="Inputs">
				...
				<xsl:element name="Control">
					...
					<xsl:element name="Mortgage">
						...
						<!-- Create an ElementGroup for each MortgageLoan -->
						<xsl:if test="//MORTGAGELOANLIST/MORTGAGELOAN">
							<xsl:element name="ADDITIONALBORROWING">
								<xsl:for-each select="//MORTGAGELOANLIST/MORTGAGELOAN">
									<xsl:variable name="MLSeqNo" select="position()"/>
		
									<xsl:element name="ElementGroup">
										<!-- Element Group Id -->
										<xsl:attribute name="Id"><xsl:value-of select="concat('ExistingMortgageAccount', $MLSeqNo)"/></xsl:attribute>
										<xsl:attribute name="IsSecured">Yes</xsl:attribute>
										<!-- Element Term (in months) -->
										<xsl:attribute name="Term"><xsl:value-of select="number(ICBSCALCULATEDOUTSTANDINGTERM)"/></xsl:attribute>
										<!-- Rate Adjustment: The manual adjustment is already included in the interest rates so rate adjustment should be 0 -->
										<xsl:attribute name="RateAdjustment">0.0</xsl:attribute>
										
										<xsl:attribute name="ParameterOverrideIds"><xsl:value-of select="concat('EMA', $MLSeqNo,'-Payments')"/></xsl:attribute>
										
										<xsl:choose>
											<xsl:when test="REPAYMENTTYPE/VALIDATIONTYPELIST/VALIDATIONTYPE='P'">
											<!-- Handle Part and Part separately as it needs to be split into 2 Elements -->
												
												<!-- Capital and Interest -->
												<xsl:element name="Element">
													<xsl:attribute name="Id"><xsl:value-of select="concat('EMA', $MLSeqNo, 'Element1')"/></xsl:attribute>
													<xsl:attribute name="Type">Repayment</xsl:attribute>
													<xsl:element name="Loan">
														<xsl:attribute name="Amount"><xsl:value-of select="REMAININGCAPITALINTERESTAMOUNT"/></xsl:attribute>
														<xsl:attribute name="Frequency">0</xsl:attribute>
													</xsl:element>
												</xsl:element>
												...
												<!-- Interest Only -->
												<xsl:element name="Element">
													<xsl:attribute name="Id"><xsl:value-of select="concat('EMA', $MLSeqNo, 'Element2')"/></xsl:attribute>
													<xsl:attribute name="Type">InterestOnly</xsl:attribute>
													<xsl:element name="Loan">
														<xsl:attribute name="Amount"><xsl:value-of select="REMAININGINTERESTONLYAMOUNT"/></xsl:attribute>
														<xsl:attribute name="Frequency">0</xsl:attribute>
													</xsl:element>
												</xsl:element>	
											</xsl:when>								
											...
											<!-- All other repayment types (non part and part)-->
												<xsl:element name="Element">
													<xsl:attribute name="Id"><xsl:value-of select="concat('EMA', $MLSeqNo, 'Element1')"/></xsl:attribute>
													<xsl:attribute name="Type">
														<xsl:if test="REPAYMENTTYPE/VALIDATIONTYPELIST/VALIDATIONTYPE='I'">InterestOnly</xsl:if>
														<xsl:if test="REPAYMENTTYPE/VALIDATIONTYPELIST/VALIDATIONTYPE='C'">Repayment</xsl:if>
													</xsl:attribute>
													<xsl:element name="Loan">
														<xsl:attribute name="Amount"><xsl:value-of select="OUTSTANDINGBALANCE"/></xsl:attribute>
														<xsl:attribute name="Frequency">0</xsl:attribute>
													</xsl:element>
												</xsl:element>		
											</xsl:otherwise>
									...
									<!-- Interest Rate "table"-->
									<xsl:element name="ParameterOverride">
										<xsl:attribute name="Id"><xsl:value-of select="concat('EMA', $MLSeqNo,'-Payments')"/></xsl:attribute>
										<xsl:attribute name="Parameter">InterestRate</xsl:attribute>
										<xsl:element name="ParameterOverrideKey">
											<xsl:attribute name="Name">Duration</xsl:attribute>
											<xsl:attribute name="NumberOfValues"><xsl:value-of select="count(INTERESTRATETYPE)"/></xsl:attribute>
