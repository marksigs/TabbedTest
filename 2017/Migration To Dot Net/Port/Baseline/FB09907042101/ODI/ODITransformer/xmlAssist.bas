...
'SA      15/07/02    SYS4754 Performance. Replace all For...Each...Next with For...Next
'------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
    ...
    Dim lngListLength As Long                           '++ SA SYS4754
    lngListLength = vxmlNodeList.length - 1             '++ SA SYS4754
    'For lngNodeCount = 0 To vxmlNodeList.length - 1    '-- SA SYS4754
    For lngNodeCount = 0 To lngListLength               '++ SA SYS4754
        xmlSetSysDateToNodeAttrib vxmlNodeList.Item(lngNodeCount), strDateAttribName, False
    Next lngNodeCount
    
End Sub
Private Sub xmlParserError(ByVal vobjParseError As IXMLDOMParseError, ByVal vstrCallingFunction As String)
...
'------------------------------------------------------------------------------------------
    ...
    errThrowError vstrCallingFunction, oeXMLParserError
        
End Sub

Private Function FormatParserError(ByVal objParseError As IXMLDOMParseError) As String
...
Public Sub xmlChangeNodeName(ByRef rxmlNode As IXMLDOMNode, _
...
' Raise Errors:
'------------------------------------------------------------------------------------------
On Error GoTo xmlChangeNodeNameVbErr
    ...
    Dim intListLength As Integer        '++ SA SYS4754
    Dim intChildNodesLength As Integer  '++ SA SYS4754
    ...
    If rxmlNode.nodeName = vstrOldName Then
        ...
        If objNewNode.nodeType = NODE_ELEMENT Then
            ...
            intListLength = rxmlNode.Attributes.length - 1              '++ SA SYS4754
            'For intAttribIndex = 0 To rxmlNode.Attributes.length - 1   '-- SA SYS4754
            For intAttribIndex = 0 To intListLength                     '++ SA SYS4754
    ...
    intChildNodesLength = rxmlNode.childNodes.length - 1                '++ SA SYS4754
    'For intChildIndex = 0 To rxmlNode.childNodes.length - 1            '-- SA SYS4754
...
Dim intNodeCount As Integer             '++ SA SYS4754
Dim intListLength As Integer            '++ SA SYS4754
...
strFunctionName = "xmlCreateElementRequestFromNode"
    ...
    If Not vxmlNode Is Nothing Then
        ...
        intListLength = vxmlNode.Attributes.length - 1      '++ SA SYS4754
        'For Each xmlNode In vxmlNode.Attributes            '-- SA SYS4754
        For intNodeCount = 0 To intListLength               '++ SA SYS4754
            'xmlCopyAttribute vxmlNode, xmlRequestNode, xmlNode.nodeName    '-- SA SYS4754
            xmlCopyAttribute vxmlNode, xmlRequestNode, vxmlNode.Attributes(intNodeCount).nodeName   '++ SA SYS4754
        Next
...
Dim intAttribsLength As Integer                             '++ SA SYS4754
Dim intAttribsCount As Integer                              '++ SA SYS4754
...
strFunctionName = "xmlMakeNodeElementBased"
    ...
    If Not vxmlNode Is Nothing Then
        ...
        If intNoOfSpecifiedTags >= 0 Then
            ...
            Next
        Else
            intAttribsLength = vxmlNode.Attributes.length - 1                   '++ SA SYS4754
            'For Each xmlAttribNode In vxmlNode.Attributes                      '-- SA SYS4754
            For intAttribsCount = 0 To intAttribsLength                         '++ SA SYS4754
                'SR 12/06/01 : if node name is description of any combo value, create a new attribute
                '              to the respective node
                'strAttribName = xmlAttribNode.nodeName                          '-- SA SYS4754
                strAttribName = vxmlNode.Attributes(intAttribsCount).nodeName    '++ SA SYS4754
                If Right(strAttribName, 5) = "_TEXT" Then
                    strComboValueNodeName = Left(strAttribName, Len(strAttribName) - 5)
                    Set xmlNode2 = xmlDestParentNode.selectSingleNode("./" & strComboValueNodeName)
                    If Not xmlNode2 Is Nothing Then
                        'xmlSetAttributeValue xmlNode2, "TEXT", xmlAttribNode.Text                          '-- SA SYS4754
                        xmlSetAttributeValue xmlNode2, "TEXT", vxmlNode.Attributes(intAttribsCount).Text    '++ SA SYS4754
                    Else
                        'Set xmlNode = xmlDOMDocument.createElement(xmlAttribNode.nodeName)                 '-- SA SYS4754
                        Set xmlNode = xmlDOMDocument.createElement(vxmlNode.Attributes(intAttribsCount).nodeName)    '++ SA SYS4754
                        If Not xmlNode Is Nothing Then
                            'xmlNode.Text = xmlAttribNode.Text          '-- SA SYS4754
                            xmlNode.Text = vxmlNode.Attributes(intAttribsCount).Text    '++ SA SYS4754
                            xmlDestParentNode.appendChild xmlNode
                        End If
                    End If
                Else
...
Dim xmlAttrib As IXMLDOMAttribute
Dim intChildCount As Integer                '++ SA SYS4754
Dim intChildLength As Integer               '++ SA SYS4754
Dim intAttribsCount As Integer              '++ SA SYS4754
Dim intAttribsLength As Integer             '++ SA SYS4754
    ...
    If Not vxmlNode Is Nothing Then
        ...
        intChildLength = vxmlNode.childNodes.length - 1                     '++ SA SYS4754
        'For Each xmlNode In vxmlNode.childNodes                            '-- SA SYS4754
        For intChildCount = 0 To intChildLength                             '++ SA SYS4754
            'Check if it is a 'text' node or genuinely has child elements
            'If xmlNode.hasChildNodes Then                                  '-- SA SYS4754
            If vxmlNode.childNodes(intChildCount).hasChildNodes Then        '++ SA SYS4754
                'If xmlNode.childNodes(0).nodeName = "#text" Then           '-- SA SYS4754
                If vxmlNode.childNodes(intChildCount).childNodes(0).nodeName = "#text" Then     '++ SA SYS4754
                    blnNoChildren = True
                End If
            Else
                blnNoChildren = True
            End If
            'If blnNoChildren And Len(Trim$(xmlNode.Text)) > 0 Then         '-- SA SYS4754
            If blnNoChildren And Len(Trim$(vxmlNode.childNodes(intChildCount).Text)) > 0 Then       '++ SA SYS4754
                'Create as an attribute of parent
                'xmlParent.setAttribute xmlNode.nodeName, xmlNode.Text      '-- SA SYS4754
                xmlParent.setAttribute vxmlNode.childNodes(intChildCount).nodeName, vxmlNode.childNodes(intChildCount).Text     '++ SA SYS4754
                'Check if combo item with TEXT attribute
                intAttribsLength = vxmlNode.childNodes(intChildCount).Attributes.length - 1 '++ SA SYS4754
                'For Each xmlAttrib In xmlNode.Attributes                                   '-- SA SYS4754
                For intAttribsCount = 0 To intAttribsLength                                 '++ SA SYS4754
                    'If UCase$(xmlAttrib.nodeName) = "TEXT" Then
                    If UCase$(vxmlNode.childNodes(intChildCount).Attributes(intAttribsCount).nodeName) = "TEXT" Then    '++ SA SYS4754
                        'Create a text attribute also for the combo text
                        'xmlParent.setAttribute xmlNode.nodeName & "_TEXT", xmlAttrib.Text      '-- SA SYS4754
                        xmlParent.setAttribute vxmlNode.childNodes(intChildCount).nodeName & "_TEXT", vxmlNode.childNodes(intChildCount).Attributes(intAttribsCount).Text       '++ SA SYS4754
                    End If
                Next
            ElseIf blnConvertChild And Not blnNoChildren Then
                'Process child node recursively
                'Set xmlNode = xmlCreateAttributeBasedResponse(xmlNode, blnConvertChild)    '-- SA SYS4754
                Set xmlNode = xmlCreateAttributeBasedResponse(vxmlNode.childNodes(intChildCount), blnConvertChild)  '++ SA SYS4754
...
Dim xmlAttrib As IXMLDOMAttribute
Dim intSchemaLength As Integer              '++ SA SYS4754
Dim intSchemaCount As Integer               '++ SA SYS4754
...
strFunctionName = "xmlCreateChildRequest"
    ...
    If (Not vxmlDataNode Is Nothing) And (Not vxmlSchemaNode Is Nothing) And Len(Trim$(vstrNewName)) > 0 Then
        ...
        intSchemaLength = vxmlSchemaNode.childNodes.length - 1              '++ SA SYS4754
        'For Each xmlSchemaElement In vxmlSchemaNode.childNodes             '-- SA SYS4754
        For intSchemaCount = 0 To intSchemaLength                           '++ SA SYS4754
            'If Not xmlSchemaElement.Attributes.getNamedItem("KEYTYPE") Is Nothing Then     '-- SA SYS4754
            If Not vxmlSchemaNode.childNodes(intSchemaCount).Attributes.getNamedItem("KEYTYPE") Is Nothing Then         '++ SA SYS4754
               ' strValue = xmlSchemaElement.Attributes.getNamedItem("KEYTYPE").Text         '-- SA SYS4754
                strValue = vxmlSchemaNode.childNodes(intSchemaCount).Attributes.getNamedItem("KEYTYPE").Text            '++ SA SYS4754
                If strValue = "PRIMARY" Then
                    'Get this attribute from the data element and add it to the new node
                    'Set xmlAttrib = vxmlDataNode.Attributes.getNamedItem(xmlSchemaElement.nodeName)                            '-- SA SYS4754
                    Set xmlAttrib = vxmlDataNode.Attributes.getNamedItem(vxmlSchemaNode.childNodes(intSchemaCount).nodeName)    '++ SA SYS4754
                    If Not xmlAttrib Is Nothing Then
                        'xmlNewNode.setAttribute xmlSchemaElement.nodeName, xmlAttrib.Text      '-- SA SYS4754
                        xmlNewNode.setAttribute vxmlSchemaNode.childNodes(intSchemaCount).nodeName, xmlAttrib.Text      '++ SA SYS4754
                    End If
                End If
            End If
        Next
...
'------------------------------------------------------------------------------------------
    ...
    'Dim xmlNode As IXMLDOMNode                         '-- SA SYS4754
    Dim intChildListLength As Integer                   '++ SA SYS4754
    Dim intChildCount As Integer                        '++ SA SYS4754
    ...
    End If
     ...
     intChildListLength = xmlChildList.length - 1           '++ SA SYS4754
     'For Each xmlNode In xmlChildList                      '-- SA SYS4754
     For intChildCount = 0 To intChildListLength            '++ SA SYS4754
        'If xmlNode.nodeName <> "MESSAGE" Then              '-- SA SYS4754
        If xmlChildList(intChildCount).nodeName <> "MESSAGE" Then   '++ SA SYS4754
            'vxmlNodeToAttachTo.appendChild xmlNode.cloneNode(True) '-- SA SYS4754
            vxmlNodeToAttachTo.appendChild xmlChildList(intChildCount).cloneNode(True)  '++ SA SYS4754
        End If
     Next
    
    Set xmlChildList = Nothing
    'Set xmlNode = Nothing                                  '-- SA SYS4754
