...
'PSC    14/03/07    EP2_1732 Remove extra spaces from postcode
'------------------------------------------------------------------------------------------
Option Explicit
...
On Error GoTo OptimusAddressToOmigaExit
    ...
    'PSC 14/03/07 EP2_1732
    xmlSetAttributeValue OmigaNode, "POSTCODE", _
        FormatPostCode(xmlGetNodeText(OptimusAddressNode, "POSTALCODE/@DATA"))
...
'PSC 14/03/07 EP2_1732 - Start
Private Function FormatPostCode(ByVal strPostCode) As String

    Dim strFormatedPostcode As String
    
    strFormatedPostcode = Trim$(strPostCode)
    
    Do While InStr(1, strFormatedPostcode, "  ") > 0
        strFormatedPostcode = Replace(strFormatedPostcode, "  ", " ")
    Loop
    
    FormatPostCode = strFormatedPostcode

End Function
'PSC 14/03/07 EP2_1732 - End
