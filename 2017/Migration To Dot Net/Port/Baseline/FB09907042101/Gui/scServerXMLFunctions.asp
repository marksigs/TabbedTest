<%
/*
Workfile:		scServerXMLFunctions.asp
Copyright:		Copyright © 1999 Marlborough Stirling

Description:	Helper functions for XML parser.
				These function have been copied for use on the
				server. As such, some of the functions may not work,
				and will need testing before use. (eg. runasp, getURL).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
MARS Specific
PE		25/04/2006	MAR992		Copied scXMLFunction for server side use.
								Note that some of the functions may now not work and will need testing before use.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
function CreateActiveTag(sTagName)
{
	...
	if(this.ActiveTag != null) this.ActiveTag.appendChild(TagNew);
	else this.XMLDocument.appendChild(TagNew);
...
function CreateTag(sTagName,vText)
{
	...
	*/
	...
	if(this.ActiveTag != null) this.ActiveTag.appendChild(TagNew);
	else alert("CreateTag - No Active Tag");

	if(vText != null) TagNew.text = vText;
	...
	*/
	...
	if(this.ActiveTag != null)
	{
		...
		if(this.ActiveBeforeTag != null)
		{
			...
			if(TagList.length > 0) TagNew.text = TagList.item(0).text;
		}
	}
	else alert("CreateTag_UseBeforeText - No Active Tag");
...
function SetAttribute(sAttributeName,vAttributeValue)
{
	...
	*/
	if(this.ActiveTag != null)
	{
		if (vAttributeValue !=null) this.ActiveTag.setAttribute(sAttributeName,vAttributeValue);
		else alert("SetAttribute - Null value for attribute " + sAttributeName);
	}
	else alert("SetAttribute - No Active Tag");
...
function SetAttributeOnTag(sTagName, sAttributeName,vAttributeValue)
{
	...
	if(this.ActiveTag != null)
	{
		...
		if (TagList.length > 0) TagList.item(0).setAttribute(sAttributeName, vAttributeValue);
	}
	else alert("No Active Tag");
}
...
function CreateRequestAttributeArray(thisWindow)
{
	...
	var bIsFrame = false;
	for(var nLoop = 0; nLoop < thisWindow.parent.frames.length && !bIsFrame; nLoop++)
		if(thisWindow.parent.frames.item(nLoop).name == "omigamenu") bIsFrame = true;
...
function RemoveAttribute(sAttributeName)
{
	/*	There must be an active tag
		RF 09/11/99 Attribute value must be specified
	*/
	if(this.ActiveTag != null)
	{
		this.ActiveTag.removeAttribute(sAttributeName);		
	}
	else alert("SetAttribute - No Active Tag");	
}
...
function CreateRequestTagFromArray(AttributeArray,sOperation)
{
	...
	if (AttributeArray.length == 6) this.SetAttribute("USERAUTHORITYLEVEL",AttributeArray[5]);
...
function GetTagText(sTagName)
{
	...
	*/
	...
	{
		var TagList = this.ActiveTag.getElementsByTagName(sTagName);
		if(TagList.length > 0) return TagList.item(0).text;
	}
	else alert("GetTagText - No Active Tag");
...
function GetTagFloat(sTagName)
{
	...
	if(sValue != "") dValue = parseFloat(sValue);
	if(isNaN(dValue)) alert("GetTagFloat - Tag value is not a number")
...
function GetTagInt(sTagName)
{
	...
	if(sValue != "") iValue = parseInt(sValue);
	if (isNaN(iValue)) alert("GetTagInt - Tag value is not a number")
...
function SetTagText(sTagName, sText)
{
	...
	*/
	...
	{
		var TagList = this.ActiveTag.getElementsByTagName(sTagName);
		if(TagList.length > 0) TagList.item(0).text = sText;
	}
	else alert("GetTagText - No Active Tag");
}
...
function GetTagBoolean(sTagName)
{
	...
	*/
	...
	{
		var TagList = this.ActiveTag.getElementsByTagName(sTagName);
		if(TagList.length > 0) return this.ConvertBoolean(TagList.item(0).text);
	}
	else alert("GetTagBoolean - No Active Tag");
...
{
	if((sTagText == "0") || (sTagText == "")) return false;
	return true;
}
	...
	*/
	if(this.ActiveTag != null)
	{
		...
		if(TagList.length > 0)
		{
			...
			var iLength = TagList.item(0).attributes.length;

			for (iLoop=0; iLoop < iLength; iLoop++)
			{
				var AttributeNode = TagList.item(0).attributes.item(iLoop);
	...
	else alert("GetTagAttribute - No ActiveTag");
...
function GetAttribute(sAttributeName)
{
	...
	if(this.ActiveTag != null)
	{
		...
		var iLength = this.ActiveTag.attributes.length;

		for (iLoop=0; iLoop < iLength; iLoop++)
		{
			var AttributeNode = this.ActiveTag.attributes.item(iLoop);
			if (AttributeNode.nodeName == sAttributeName) return AttributeNode.nodeValue;
		}
	}
	else alert("GetAttribute - No Active Tag");
...
function GetNodeAttribute(xmlNode, sAttributeName)
{
	...
	for(iLoop=0; iLoop < iLength; iLoop++)
	{
		AttributeNode = xmlNode.attributes.item(iLoop);	
		if (AttributeNode.nodeName == sAttributeName) return AttributeNode.nodeValue;
	}
}
...
function RemoveActiveTag()
{
	...
	*/
	if(this.ActiveTag != null)
	{
		...
		if(TagParent.parentNode != null) this.ActiveTag = TagParent;
		else this.ActiveTag = null;
	}
	else alert("RemoveActiveTag - No Active Tag");
}
...
function CreateTagList(sTagName)
{
	...
	if(this.ActiveTag != null) TagListNew = this.ActiveTag.getElementsByTagName(sTagName);
	else TagListNew = this.XMLDocument.getElementsByTagName(sTagName);
...
function SelectTagListItem(nItem)
{
	...
	*/
	if (nItem >= 0)
	{
		...
		else alert("SelectedTagListItem - No Active Tag List");
	}
...
function SelectTag(TagParent,sTagName)
{
	...
	*/
	...
	if(TagParent != null) TagListNew = TagParent.getElementsByTagName(sTagName);
	else TagListNew = this.XMLDocument.getElementsByTagName(sTagName);
...
function GetTagListItem(nItem)
{
	...
	*/
	if(this.ActiveTagList != null)
	{
		if(this.ActiveTagList.length >= nItem + 1) return this.ActiveTagList.item(nItem);
	}
	else alert("SelectedTagListItem - No Active Tag List");
...
function LoadXML(sXML)
{
	...
	if(sXML != null) this.XMLDocument.loadXML(sXML);
	else this.XMLDocument.loadXML("");
}
...
function SelectUPDATE_Before()
{
	...
	if(!bReturn) this.ActiveTag = null;
...
function CreateFragment()
{
	...
	*/
	...
	else alert("CreateFragment - No Active Tag");
...
{
	if (sFileName == "") sFileName = "c:\\script.xml";
...
function ReadXMLFromFile(sFileName)
{
	...
	if (sFileName != "")
	{
		...
		else alert("file " + sFileName + " not found");
	}
	return (sXMLString);
}
...
function GetComboLists(thisDocument,sGroupList)
{
	...
	{
		for(var nLoop = 0;nLoop < sGroupList.length;nLoop++) this.CreateTag("LISTNAME",sGroupList[nLoop]);
		this.RunASP(thisDocument,"GetComboList.asp");
		return this.IsResponseOK();
	}
	else alert("GetComboLists - No ListNames specified");
...
function GetComboIdForValidation(sListName, sValidation, XMLCombo, thisDocument)
{
	...
	*/
	...
	if(arguments.length < 3) bBuildComboXML = true ;
	else 
	{
		if(XMLCombo == null) bBuildComboXML = true ;
	}
	
	if(bBuildComboXML)
	{	
		if(arguments.length < 4) return "" ;
		if(thisDocument == null) return "" ;
	...
	{	
		...
		XML.CreateTagList("VALIDATIONTYPE");
		for(var nListLoop = 0; nListLoop < XML.ActiveTagList.length; nListLoop++)
			if(XML.ActiveTagList.item(nListLoop).text == sValidation) return sComboValueId ;
	}
}
...
function PopulateCombo(thisDocument,refFieldId,sListName,bAddSELECT,bAppend)
{
	...
	if(refFieldId.tagName == "SELECT")
	{
		...
		{
			if(XMLFragment != null) return this.PopulateComboFromXML(thisDocument,refFieldId,XMLFragment,bAddSELECT,vUndefined,vUndefined,vUndefined,vUndefined,bAppend);
		}
		else
		{
			if(XMLFragment != null) return this.PopulateComboFromXML(thisDocument,refFieldId,XMLFragment,bAddSELECT);
		}
	}
	else alert("PopulateComboFromDB - field must be a combo");
...
function PopulateComboFromXML(thisDocument,refFieldId,XMLSource,bAddSELECT,bAttibuteBase,ValueActiveTag,ValueIDTagName,ValueTagName,bAppend)
{
	...
	*/
	...
	if(refFieldId.tagName == "SELECT")
	{
		...
		else alert("PopulateComboFromXML - document fragment is null");
	}
	else alert("PopulateComboFromXML - field must be a combo");
...
function IsInComboValidationXML(ValidationList)
{
	...
	for(var nListLoop = 0;nListLoop < this.ActiveTagList.length;nListLoop++)
		for (var nValLoop = 0;nValLoop < ValidationList.length;nValLoop++)
			if (this.ActiveTagList.item(nListLoop).text == ValidationList[nValLoop]) return true;
...
function CheckResponse(sErrorTypes)
{
	...
	*/
	...
	if (TagRESPONSE != null)
	{
		if (this.GetAttribute("TYPE") == "SUCCESS") ErrorReturn[0] = true;
	...
	if(!ErrorReturn[0] && ErrorReturn[1] == null) alert(sErrorMessage + "\nSource: " + sErrorSource + "\nVersion: " + sVersion);
...
function GetGlobalParameterBoolean(thisDocument,sParameterName)
{
	...
	if(this.IsResponseOK())
	{
		...
		if(this.SelectTagListItem(0)) return this.GetTagBoolean("BOOLEAN");
	}
...
function GetGlobalParameterAmount(thisDocument,sParameterName)
{
	...
	{
		this.CreateTagList("GLOBALPARAMETER");
		if(this.SelectTagListItem(0)) return this.GetTagText("AMOUNT");
	}
...
function GetGlobalParameterString(thisDocument,sParameterName)
{
	...
	{
		this.CreateTagList("GLOBALPARAMETER");
		if(this.SelectTagListItem(0)) return this.GetTagText("STRING");
	}
...
function SelectNodes(sPattern)
{
	...
	if(this.ActiveTag != null) TagListNew = this.ActiveTag.selectNodes(sPattern);
	else TagListNew = this.XMLDocument.selectNodes(sPattern);
...
{
	if(this.ActiveTag != null) this.ActiveTag = this.ActiveTag.selectSingleNode(sPattern);
	else this.ActiveTag = this.XMLDocument.selectSingleNode(sPattern);
