...
Description:   CIFAS Summary Screen
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
History:

Prog	Date		Descrip
JR		5/12/00		Screen Design
JR		21/12/00	Added Functionality
...
HMA     09/12/2003  BMIDS660    Change screen title.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*/
%>
<%
...
%>
<% /* BMIDS660 Change title */ %>
<HEAD>
	<meta NAME="GENERATOR" Content="Microsoft Visual Studio 6.0">
	<link href="stylesheet.css" rel="STYLESHEET" type="text/css"> 
	<title>CIFAS do not reject - refer to validation</title>
</head>
<body>
...
<div style="HEIGHT: 135px; LEFT: 10px; POSITION: absolute; TOP: 300px; WIDTH: 430px" class="msgGroup">
	<span style="LEFT: 10px; POSITION: absolute; TOP: 10px" class="msgLabel">
		Reference
		<span style="LEFT: 130px; POSITION: absolute; TOP: -3px">
			<input id="txtFBCIFASReference" name="Reference" maxlength="9" style="POSITION: absolute; WIDTH: 70px" class="msgReadOnly" readonly>
		</span>
	</span>
	<span style="LEFT: 10px; POSITION: absolute; TOP: 35px" class="msgLabel">
		Full Supplying Company
		<span style="LEFT: 130px; POSITION: absolute; TOP: -3px">
			<input id="txtFBCIFASSupplyCoName" name="SupplyingCompany" maxlength="40" style="POSITION: absolute; WIDTH: 250px" class="msgReadOnly" readonly>
		</span>
	</span>
	<span style="LEFT: 10px; POSITION: absolute; TOP: 60px" class="msgLabel">
		Fraud Category
		<span style="LEFT: 130px; POSITION: absolute; TOP: -3px">
			<input id="txtFBCIFASFraudCategory" name="FraudCategory" maxlength="40" style="POSITION: absolute; WIDTH: 250px" class="msgReadOnly" readonly>
		</span>
	</span>	
	<span style="LEFT: 10px; POSITION: absolute; TOP: 85px" class="msgLabel">
		Date Info Supplied
		<span style="LEFT: 130px; POSITION: absolute; TOP: -3px">
			<input id="txtFBCIFASSupplyDate" name="DateInfoSupplied" maxlength="10" style="POSITION: absolute; WIDTH: 70px" class="msgReadOnly" readonly>
		</span>
	</span>
	<span style="LEFT: 10px; POSITION: absolute; TOP: 110px" class="msgLabel">
		Date Info Due To Expire
		<span style="LEFT: 130px; POSITION: absolute; TOP: -3px">
			<input id="txtFBCIFASExpiryDate" name="Reference" maxlength="10" style="POSITION: absolute; WIDTH: 70px" class="msgReadOnly" readonly>
		</span>
	</span>
</div>
<% /* BM0063 - Put <input> and <label> on the same line for radio buttons to fix Read Only problem in IE5.5 */%>
<div style="HEIGHT: 70px; LEFT: 10px; POSITION: absolute; TOP: 440px; WIDTH: 430px" class="msgGroup">
	<span id=optNoticesOfCorrection name=NoticesOfCorrection style="TOP: 20px; LEFT: 10px; POSITION: ABSOLUTE" class="msgLabel">
		Notices of Correction
		<span style="TOP: 3px; LEFT: 130px; POSITION: ABSOLUTE">
			<input id="optNoticesOfCorrectionYes" name="NoticesOfCorrection" type="radio" value="1" tabindex=-1><label for="optNoticesOfCorrectionYes" class="msgLabel">Yes</label>
		</span> 
		<span style="TOP: 3px; LEFT: 200px; POSITION: ABSOLUTE">
			<input id="optNoticesOfCorrectionNo" name="NoticesOfCorrection" type="radio" value="0" tabindex=-1><label for="optNoticesOfCorrectionNo" class="msgLabel">No</label>
		</span> 
	</span>	
...
<%/* Main Buttons */ %>
<div id="msgButtons" style="HEIGHT: 19px; LEFT: 8px; POSITION: absolute; TOP: 530px; WIDTH: 420px">
...
<% /* Added by automated update TW 09 Oct 2002 SYS5115 */ %>
<!-- #include FILE="attribs/ra035Attribs.asp" -->
...
var m_iaFBCIFASSequence = new Array(); //Array containing the header sequence numbers of public info for current customer
...
function PopulateScreen()
{
	...
	XML.CreateActiveTag("FULLBUREAUCIFAS");
	...
	XML.RunASP(document,"GetCurrentCIFASSummary.asp");
	
	if(!XML.IsResponseOK())
	{
		alert('Error retreiving CIFAS summary');
		return ;
	}
	...
	for (iCount=0; iCount < XMLFBResults.ActiveTagList.length; ++iCount)
	{
		...
		if(sFBBLockId == m_sFBBlockId)
		{
			...
			for(iFBCIFASCount = 0; iFBCIFASCount < XML.ActiveTagList.length; ++iFBCIFASCount)
			{
				XML.SelectTagListItem(iFBCIFASCount);
				...
				{
					m_iaFBCIFASSequence[m_iaFBCIFASSequence.length] = iFBCIFASCount ;
					GetRelatedAddress(XMLFBResults.GetTagText("FBADDRESSINDICATOR")) ;
					break ;
				}
			}		
			// Find the current public info node to be displayed
			if(sFBBLockId == m_sFBBlockId && sFBHeaderSeqNo == m_sFBHeaderSeqNo) iCurrentBlock = m_iaFBCIFASSequence.length - 1 ;
		}
	}
	FillScreen() ;
}
...
function FillScreen()
{
	// Select the nodes (in FullBureau results, CIFAS) from which the data is to be displayed in the screen
	XMLFBResults.SelectTagListItem(m_iaFBResultsSequence[iCurrentBlock]);
	XML.SelectTagListItem(m_iaFBCIFASSequence[iCurrentBlock]);
	...
	// Other Details
	frmScreen.txtFBCIFASReference.value		= XML.GetTagText("FBCIFASREFERENCE");
	frmScreen.txtFBCIFASSupplyCoName.value	= XML.GetTagText("FBCIFASSUPPLYCONAME");
	frmScreen.txtFBCIFASFraudCategory.value = XML.GetTagText("FBCIFASFRAUDCATEGORY");
	frmScreen.txtFBCIFASSupplyDate.value	= XML.GetTagText("FBCIFASSUPPLYDATE");
	frmScreen.txtFBCIFASExpiryDate.value	= XML.GetTagText("FBCIFASEXPIRYDATE");
...
function ClearScreen()
{
	...
	// Other Details
	frmScreen.txtFBCIFASReference.value				= "";
	frmScreen.txtFBCIFASSupplyCoName.value			= "";
	frmScreen.txtFBCIFASFraudCategory.value			= "";
	frmScreen.txtFBCIFASSupplyDate.value			= "";
	frmScreen.txtFBCIFASExpiryDate.value			= "";
	
}

function DisableNavigationButtons()
{
	if(m_iaFBCIFASSequence.length == 1 || m_iaFBCIFASSequence.length == 0)
	{
	...
	else 
	{
		...
		{
			if(parseInt(iCurrentBlock) + 1 == m_iaFBCIFASSequence.length)
			{
