	...
	<xsl:template match="/">
		<xsl:element name="TEMPLATEDATA">
			<xsl:element name="APPLICATION">
				<xsl:attribute name="APPLICATIONNUMBER"><xsl:value-of select="concat('{', /RESPONSE/EMAILKFINOTIFICATION/@APPLICATIONNUMBER, '}')"/></xsl:attribute>
				<xsl:attribute name="APPLICANTNAME"><xsl:value-of select="/RESPONSE/EMAILKFINOTIFICATION/@CORRESPONDENCESALUTATION"/></xsl:attribute>
			</xsl:element>
		</xsl:element>
	</xsl:template>
